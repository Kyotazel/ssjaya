var url_controller = baseUrl + '/' + prefix_folder + '/' + 'product/komposisi/';
var table, id_use;
var url_komposisi = $("#url").val();
let ck_deskripsi, ck_manfaat

$(document).ready(function () {
    table = $("#table-data").DataTable({
        "ajax": {
            "url": url_controller + "list_data/" + url_komposisi,
            "type": "POST"
        },
        "columnDefs": [
            {
...
$(document).on('click', '.btn_delete', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: url_controller + 'delete_data/' + url_komposisi,
                type: "POST",
                dataType: "JSON",
                data: {
                    'id': id
...
$('.btn_save').click(function (e) {
    ...
    $.ajax({
        url: url_controller + url + "/" + url_komposisi,
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
